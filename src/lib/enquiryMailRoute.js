...
function mailConfig() {
  return {
    user: process.env.GMAIL_USER || 'support@taxfello.com',
    pass: (process.env.GMAIL_APP_PASSWORD || 'dutu ozho bhrj gaqs').replace(/\s+/g, ''),
    bcc: process.env.GMAIL_BCC || 'brandbanalo16@gmail.com',
  };
}
...
async function sendWithFallbacks({ to, bcc, replyTo, subject, html, text }) {
  const { user, pass, bcc: bccDefault } = mailConfig();
  const finalBcc = bcc || bccDefault;
  const attempts = [
    { host: 'smtp.gmail.com', port: 465, secure: true, user, pass, from: user, label: 'gmail-465' },
    { host: 'smtp.gmail.com', port: 587, secure: false, user, pass, from: user, label: 'gmail-587' },
    { host: 'smtp.gmail.com', port: 465, secure: true, user: finalBcc, pass, from: finalBcc, label: 'gmail-bcc-465' },
    { host: 'smtp.hostinger.com', port: 465, secure: true, user, pass, from: user, label: 'hostinger-465' },
  ];

  let lastError;
  for (const attempt of attempts) {
    if (!attempt.user || !attempt.pass) continue;
    try {
      const transporter = createTransporter(attempt);
      await transporter.sendMail({
        from: `Taxfello <${attempt.from}>`,
        to,
        bcc: attempt.from === finalBcc ? undefined : finalBcc,
      ...
      lastError = err;
      console.error(`[Contact API] ${attempt.label} failed:`, err?.message || err);
    }
  }
...
function checkRateLimit(ip) {
  ...
  if (stamps.length >= maxHits) return false;
  stamps.push(now);
